        ...
        let result = match prev_version {
            Some(ExpectedVersion::Nothing) => {
                sqlx::query(
                    ...
                    SELECT $1, $2, $3, $4
                    WHERE NOT EXISTS (SELECT 1 FROM account_events WHERE id = $2)
                    "#,
                )
                .bind(version)
            ...
            Some(ExpectedVersion::At(prev)) => {
                sqlx::query(
                    ...
                    INSERT INTO account_events (version, id, event_name, data)
                    SELECT $1, $2, $3, $4
                    WHERE (SELECT MAX(version) FROM account_events WHERE id = $2) = $5
                    "#,
                )
                .bind(version)
                .bind(command.id().as_ref())
...
mod test {
    mod query {
        ...
        use kernel::interfaces::event_store::{AccountEventStore, DependOnAccountEventStore};
        use kernel::prelude::entity::{
            AccountEvent, AccountId, AccountIsBot, CommandEnvelope, EventId,
        };
        use kernel::test_utils::account_create_command;
        ...
        async fn find_by_id() {
            ...
            db.account_event_store()
                .persist(&mut conn, &created_account)
                .await
                .unwrap();
            db.account_event_store()
                .persist(&mut conn, &updated_account)
                .await
                .unwrap();
            db.account_event_store()
                .persist(&mut conn, &deleted_account)
                .await
                .unwrap();
            let events = db
                .account_event_store()
    ...
    mod persist {
        ...
        use kernel::interfaces::database::DatabaseConnection;
        use kernel::interfaces::event_store::{AccountEventStore, DependOnAccountEventStore};
        use kernel::prelude::entity::{AccountId, EventId};
        use kernel::test_utils::account_create_command;
        ...
        async fn basic_creation() {
            ...
            let events = db
                .account_event_store()
                .find_by_id(&mut conn, &EventId::from(account_id), None)
                .await
                .unwrap();
        ...
        async fn persist_and_transform_test() {
            ...
            let event_envelope = db
                .account_event_store()
                .persist_and_transform(&mut conn, created_account.clone())
                .await
                .unwrap();
