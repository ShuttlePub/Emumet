...
impl OutboxActivityRepository for PostgresOutboxActivityRepository {
    ...
    async fn create(
        ...
        let id: i64 = sqlx::query_scalar(
            r#"
            INSERT INTO outbox_activities (account_id, activity_id, activity_type, object_json, created_at, delivered_at, attempted_at, error)
        ...
        .bind(activity.delivered_at)
        .bind(activity.attempted_at)
        .bind(&activity.error)
        .fetch_one(con)
        .await
        .convert_error()?;
...
mod tests {
    ...
    fn outbox_activity(account_id: &AccountId, activity_type: &str) -> OutboxActivity {
    ...
    async fn pending_ids_for(
        database: &PostgresDatabase,
        executor: &mut PostgresConnection,
        account_id: &AccountId,
    ) -> Vec<OutboxActivityId> {
        database
            .outbox_activity_repository()
            .find_pending_deliveries(executor, 100)
            .await
            .unwrap()
            .into_iter()
    ...
    async fn create_and_find_by_account_id_returns_stored_activity() {
        ...
        let mut activity = outbox_activity(&account_id, "Create");
    ...
    async fn create_returns_generated_id() {
        ...
        let mut executor = database.connection().await.unwrap();
        let account_id = AccountId::default();
        let activity = outbox_activity(&account_id, "Create");
        ...
        assert!(id > 0);
        let pending = pending_ids_for(&database, &mut executor, &account_id).await;
    ...
    async fn pending_deliveries_excludes_delivered() {
        ...
        let first = database
            .outbox_activity_repository()
            .create(&mut executor, &outbox_activity(&account_id, "Follow"))
            .await
            .unwrap();
        let second = database
            .outbox_activity_repository()
        ...
        database
            .outbox_activity_repository()
            .mark_delivered(&mut executor, &first)
            .await
            .unwrap();
    ...
    #[test_with::env(DATABASE_URL)]
    #[tokio::test]
    async fn mark_delivered_sets_delivered_and_clears_attempt() {
        kernel::ensure_generator_initialized();
        let database = PostgresDatabase::new().await.unwrap();
        let mut executor = database.connection().await.unwrap();
        let account_id = AccountId::default();
        let id = database
            .outbox_activity_repository()
            .create(&mut executor, &outbox_activity(&account_id, "Create"))
            .await
            .unwrap();
        database
            .outbox_activity_repository()
        ...
        assert_eq!(stored.len(), 1);
        assert!(stored[0].delivered_at.is_some());
        assert!(stored[0].attempted_at.is_none());
        assert!(stored[0].error.is_none());
    }

    #[test_with::env(DATABASE_URL)]
    #[tokio::test]
    async fn mark_delivery_attempt_records_error() {
        kernel::ensure_generator_initialized();
        let database = PostgresDatabase::new().await.unwrap();
        let mut executor = database.connection().await.unwrap();
        let account_id = AccountId::default();
        ...
        let pending = database
            .outbox_activity_repository()
            .find_pending_deliveries(&mut executor, 100)
            .await
            .unwrap();
        let stored = pending.iter().find(|activity| activity.id == id).unwrap();
        assert!(stored.attempted_at.is_some());
        assert_eq!(stored.error.as_deref(), Some("temporary failure"));
        assert!(stored.delivered_at.is_none());
    }

    #[test_with::env(DATABASE_URL)]
    #[tokio::test]
    async fn find_by_account_id_excludes_undelivered() {
        kernel::ensure_generator_initialized();
        let database = PostgresDatabase::new().await.unwrap();
        let mut executor = database.connection().await.unwrap();
        let account_id = AccountId::default();
    ...
    #[test_with::env(DATABASE_URL)]
    #[tokio::test]
    async fn count_by_account_id_excludes_undelivered() {
        kernel::ensure_generator_initialized();
        let database = PostgresDatabase::new().await.unwrap();
        let mut executor = database.connection().await.unwrap();
        let account_id = AccountId::default();
