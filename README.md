...
ActivityPub 連携の E2E テストは Iceshrimp インスタンスを含む Docker Compose 環境で実行します。
...
Iceshrimp (`iceshrimp.dev/iceshrimp/iceshrimp:v2026.5.1`) を含む全サービスが起動します。
...
e2e/
├── run-ap-e2e.sh            # 自動ランナー (AP E2E)
├── certs/
│   ├── gen-cert.sh          # 証明書生成 (3 SAN ドメイン)
