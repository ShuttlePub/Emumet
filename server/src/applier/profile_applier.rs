...
use application::service::profile::UpdateProfile;
use error_stack::ResultExt;
use kernel::interfaces::signal::Signal;
use kernel::prelude::entity::ProfileId;
use rikka_mq::define::redis::mq::RedisMessageQueue;
use rikka_mq::mq::MessageQueue;
use std::sync::Arc;
use uuid::Uuid;

pub(crate) struct ProfileApplier(RedisMessageQueue<Arc<Handler>, Uuid, ProfileId>);
...
impl ProfileApplier {
    pub fn new(handler: Arc<Handler>) -> Self {
        let queue = RedisMessageQueue::new(
            ...
            handler,
            "profile_applier".to_string(),
            rikka_mq::config::MQConfig::default(),
            Uuid::new_v4,
            |handler: Arc<Handler>, id: ProfileId| async move {
                handler
                    .pgpool()
                    .update_profile(id)
                    .await
...
impl Signal<ProfileId> for ProfileApplier {
    async fn emit(&self, signal_id: ProfileId) -> error_stack::Result<(), kernel::KernelError> {
        self.0
